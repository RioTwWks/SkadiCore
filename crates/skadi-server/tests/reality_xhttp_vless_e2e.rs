//! E2E: VLESS over REALITY + XHTTP (stream-one) с реальным клиентом Xray-core.

mod common;

use common::xray::{
    ensure_xray_binary, socks5_connect, RealityTestKeys, XhttpClientSettings, XrayClient,
};
...
#[tokio::test]
async fn vless_over_reality_xhttp_with_xray_client() {
    ...
    let xhttp = XhttpClientSettings {
        path: XHTTP_PATH.to_string(),
        mode: "stream-one".to_string(),
    };
    let mut xray = XrayClient::start_xhttp(
        &xray_bin,
        "127.0.0.1",
        proxy_addr.port(),
        TEST_USER_ID,
    ...
    let payload = b"hello vless+reality+xhttp via xray";
