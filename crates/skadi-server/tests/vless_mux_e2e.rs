...
fn build_mux_new_frame(target: SocketAddr, payload: &[u8]) -> Vec<u8> {
    let meta = MuxMeta {
...
#[tokio::test]
async fn vless_mux_tcp_connect_and_relay() {
    ...
    let payload = b"hello via mux";
    let frame = build_mux_new_frame(
