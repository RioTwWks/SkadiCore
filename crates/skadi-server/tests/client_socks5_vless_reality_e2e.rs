//! E2E: skadi-client (SOCKS5) → VLESS+REALITY (native rustls) → skadi-server.
...
async fn spawn_reality_vless_server(
...
async fn socks5_connect(target_ip: Ipv4Addr, target_port: u16, proxy: std::net::SocketAddr) {
    ...
    let payload = b"hello via skadi-client+reality";
...
#[tokio::test]
async fn client_socks5_to_vless_reality_relay() {
    ...
    let server_shutdown = spawn_reality_vless_server(proxy_addr, &keys, dest_addr).await;
