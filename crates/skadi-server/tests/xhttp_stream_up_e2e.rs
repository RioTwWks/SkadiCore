//! VLESS over TLS + XHTTP stream-up (GET downlink + POST uplink).
...
const SESSION_ID: &str = "e2e-stream-up-session";
...
async fn write_chunk(stream: &mut tokio_rustls::client::TlsStream<TcpStream>, data: &[u8]) {
    let hdr = format!("{:x}\r\n", data.len());
...
#[tokio::test]
async fn vless_over_xhttp_stream_up_handshake() {
    ...
    let shutdown = spawn_xhttp_server(proxy_addr, &cert_path, &key_path, "stream-up").await;
    ...
    let post_path = format!("/xhttp/{}", SESSION_ID);
    ...
    assert!(
        post_headers.starts_with(b"HTTP/1.1 200"),
        "POST: {}",
        String::from_utf8_lossy(&post_headers)
    );
    ...
    write_chunk(&mut post_tls, &vless_req).await;
    write_chunk(&mut post_tls, b"").await;
    post_tls.shutdown().await.ok();
