...
- [ ] Маскирование Bearer-токена в логах и метриках
- [ ] Аудит изменений через gRPC API
...
- [ ] `scripts/probe-test.sh` — JA3/JA4 и тайминги vs реальный dest
...
- [ ] Подпись релизов (`cosign` / `minisign`) + инструкция в README
...
- [ ] Полная поддержка IPv6 в `listen` (`[::]:443`) или явное предупреждение в доках
...
- [ ] XHTTP-транспорт (+ REALITY, `publicKey` в realitySettings)
...
- [ ] `[::]:443` + корректные клиентские конфиги IPv6
